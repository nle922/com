...
//savesome firm_id using "./SES1459263 Team Folder/V2/Establishment Link Updates/firmids", replace
...
foreach industry in `industries'{
	preserve
		...
		local keep = 1
		local counter = 0 
		qui do "./SES1459263 Team Folder/V2/Establishment Link Updates/Extra/collapse_firmids_test"
		local number = _N
		while `keep'{
			local counter = `counter' + 1
			qui do "./SES1459263 Team Folder/V2/Establishment Link Updates/Extra/collapse_firmids_test" 2 1 1
			if `number' == _N | `counter' > 0{
				local keep = 0
				di "Industry `industry' Required `counter' Steps"
			}
			else{
				local number = _N
			}
...
//TOO SLOW TO RERUN ON FULL SAMPLE
//qui do "./SES1459263 Team Folder/V2/Establishment Link Updates/collapse_firmids" 2 1 1

drop establishment_id*
//savesome firm_id using "./SES1459263 Team Folder/V2/Establishment Link Updates/collapsed", replace
